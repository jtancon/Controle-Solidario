...
function PerfilONG() {
  ...
  const verificarForcaSenha = s => {
    if (!s) return "";
    if (/^(?=.*[a-zA-Z])(?=.*\d)(?=.*[@$!%*?&]).{8,}$/.test(s)) return "forte";
    if (/^(?=.*[a-zA-Z])(?=.*\d).{6,}$/.test(s)) return "media";
    return "fraca";
  };
  ...
  const confirmarEdicao = async () => {
    ...
    if (senha) {
      ...
      try { const auth = getAuth(); await updatePassword(auth.currentUser, senha); toast.success("Senha atualizada!"); }
      catch { return toast.error("Erro na atualização."); }
    }
    try { await updateDoc(doc(db,'ongs',user.uid),dados); toast.success('Atualizado!'); setEditando(false); }
    catch { toast.error('Falha ao atualizar.'); }
  };

  const handleConfirmarDelete = async () => {
    setMostrarConfirmacao(false); setEditando(false);
    await deletarConta(); toast.error('Conta deletada.'); signOut(); sessionStorage.clear();
  };
        ...
        <div className="perfil-right">
          ...
          {!editando ? (
            <div className="perfil-visualizacao">
              {['nome','cnpj','endereco','telefone','representante','cep','email'].map(key=> (
